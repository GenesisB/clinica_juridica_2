<?php 
	...
	class modelo_busquedas extends CI_Model {
		...
		public function buscar_causas_individual(){
				...
				$condicion_vigencia = "AND A.TERMINO IS NULL";
				$query_tipo = $this->db->query("select tipo_usuario FROM usuarios WHERE rut = '$user';");
				$consulta_persona="";
				...
				if($tipo==='A'){    
					$consulta_persona = "B.rut = '$user'";
				}else if($tipo==='P') {
					$consulta_persona = "A.RUT_ABOGADO = '$user'";
				}else{
		...
		public function sede_actual(){
			$user=$_SESSION['login_user'];
			$query = $this->db->query("SELECT sede FROM usuarios WHERE rut = '$user';");
		...
		public function traer_sedes(){
			$user=$_SESSION['login_user'];
			$query = $this->db->query("SELECT u.id_sede, s.nombre_sede FROM usuario_sede u left outer join sedes s on s.id_sede = u.id_sede where rut = '$user' ");
			return $query;
		}
	}
?>